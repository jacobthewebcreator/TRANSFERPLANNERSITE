...
export default function App() {
  ...
  // Group courses by term for organized display
  ...
  // --- AI CHAT HANDLER ---
  const handleWidgetChatSubmit = async (textToSend) => {
  ...
  const triggerGoogleLogin = async () => {
    ...
    try {
      ...
      setAuthError(err.message || "Interrupted login process.");
    }
  };
        ...
        <div className="flex flex-col w-full h-full overflow-y-auto bg-white">
          <header className="max-w-7xl mx-auto w-full px-6 py-5 flex justify-between items-center shrink-0">
            ...
            <button
              ...
              className="bg-indigo-50 text-indigo-700 font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-indigo-100 transition-all"
            >
              Sign In
            </button>
          </header>
          ...
          <main className="max-w-6xl mx-auto w-full px-6 flex-1 flex flex-col md:flex-row items-center justify-center gap-12 py-16">
            <div className="flex-1 space-y-6 text-center md:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                <button
                  onClick={() => setCurrentView('auth')}
                  className="bg-indigo-600 text-white font-bold px-8 py-4 rounded-2xl hover:bg-indigo-700 transition-all shadow-lg flex items-center justify-center gap-2 text-base"
                >
                  Get Started Free <ArrowRight className="w-5 h-5" />
                </button>
                <button
                  onClick={() => setCurrentView('checkout')}
                  className="bg-white text-slate-800 border border-slate-200 font-bold px-8 py-4 rounded-2xl hover:bg-slate-50 transition-all text-base shadow-sm"
                >
        ...
        <div className="flex justify-center items-center w-full h-full bg-slate-50 p-6">
          <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200 p-8 shadow-xl space-y-6 text-center">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 text-indigo-600 font-black text-3xl">
                <BookMarked className="w-9 h-9" /><span>PathMaker</span>
              </div>
            ...
            <div className="space-y-4">
              <button
                ...
                className="w-full py-3.5 px-4 border border-slate-200 rounded-2xl font-bold text-slate-700 bg-white hover:bg-slate-50 flex items-center justify-center gap-3 transition-all shadow-sm"
              >
              ...
              <button
                ...
                className="w-full py-3.5 px-4 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-2xl shadow-sm transition-all text-sm"
              >
                Explore as Sandbox Guest
              </button>
            </div>
            {authError && (
              <p className="text-xs text-rose-500 bg-rose-50 p-2.5 rounded-lg font-medium">{authError}</p>
            )}
            <button
        ...
        <div className="flex justify-center items-center w-full h-full bg-slate-50 p-6 overflow-y-auto">
          <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6">
              <button
                onClick={() => user ? setCurrentView('app') : setCurrentView('landing')}
              ...
              <div className="space-y-2.5">
                {[
                  'Term-by-term checklist grouping options',
                  'Direct premium automated major requirements checklist',
                  'Floating AI Transfer Advisor assistant module',
                  'Real-time database backup synchronization patterns'
                ].map((feat, idx) => (
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-xl space-y-6">
              ...
              <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                <div>
                  <h3 className="font-extrabold text-slate-800">Pro Subscription</h3>
                  <p className="text-xs text-slate-400">Cancel anytime, fully secured.</p>
                </div>
              ...
              <button
                onClick={triggerSubscriptionSuccess}
          ...
          <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col transform transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="h-16 border-b border-slate-100 px-6 flex items-center justify-between">
              <div className="flex items-center gap-2 text-indigo-600 font-extrabold text-xl">
                <BookMarked className="w-6 h-6" /><span>PathMaker</span>
              </div>
              ...
              {['Dashboard', 'My Transcript', 'TAG Eligibility', 'Target Schools', 'Settings'].map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-semibold transition-all ${isActive ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                    <span>{item}</span>
                  </button>
                );
            ...
            <header className="h-16 border-b border-slate-200 bg-white px-6 flex items-center justify-between shrink-0">
              ...
              {isSaving && (
                <span className="text-xs font-bold text-slate-400 flex items-center gap-1">
                  <Check className="w-4 h-4 text-emerald-500" /> Saved to Cloud
                </span>
              )}
            </header>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex items-center gap-4">
                      ...
                      <div>
                        <span className="text-xs font-bold text-slate-400 block uppercase">UC Prerequisite GPA</span>
                        <p className="text-3xl font-black text-slate-800">{calculatedMetrics.transferGPA.toFixed(2)}</p>
                      </div>
                    </div>
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-lg">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between text-xs font-bold text-slate-400 uppercase">
                          <span>Total Semester Units</span>
                          <span>{calculatedMetrics.totalTrackedUnits} / 60</span>
                        </div>
                        <p className="text-2xl font-black text-slate-800 mt-1">
                          {calculatedMetrics.completedUnits} <span className="text-xs text-slate-400 font-normal">Completed</span>
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                    ...
                    <form onSubmit={handleAddNewCourse} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-4 items-end">
                      ...
                      <div className="md:col-span-1">
                        <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 rounded-xl text-xs">
                        ...
                        <div key={termKey} className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                            <span className="text-xs font-extrabold text-slate-700 flex items-center gap-1.5">
                              <Calendar className="w-3.5 h-3.5 text-indigo-500" /> {termKey} Portfolio
                            </span>
                          </div>
                              ...
                              {list.map(c => (
                                <tr key={c.id} className="hover:bg-slate-50/50">
                                  ...
                                  <td className="px-6 py-3.5 font-bold">
                                    <span className={`px-2 py-0.5 rounded ${c.status === 'Completed' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                                      {c.grade}
                                    </span>
                                  </td>
                                  <td className="px-6 py-3.5 text-right">
                                    <button onClick={() => handleDeleteCourse(c.id)} className="text-slate-400 hover:text-rose-600">
              ...
              {activeTab === 'TAG Eligibility' && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div>
                      <h3 className="font-extrabold text-lg text-slate-800">Guaranteed System Matrix Validation</h3>
                      <p className="text-xs text-slate-400 mt-1">Cross-referencing your live {calculatedMetrics.transferGPA.toFixed(2)} GPA against the 2026-2027 parameters.</p>
                    </div>
                    <select
                      className="px-3 py-2 border border-slate-200 rounded-xl bg-white text-xs font-bold text-slate-700 focus:outline-none"
                      value={majorCategory}
                      onChange={(e) => { setMajorCategory(e.target.value); saveUserData({ majorCategory: e.target.value }); }}
                    >
                      <option>General / Humanities / Social Sciences</option>
                      <option>Engineering / Computer Science</option>
                      <option>Business / Economics</option>
                      <option>Natural / Biological Sciences</option>
                    </select>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {tagEvaluations.map(tag => {
                      ...
                      return (
                        <div
                          key={tag.campus}
                          className={`bg-white border rounded-2xl p-5 space-y-4 shadow-sm flex flex-col justify-between ${isEligible ? 'border-emerald-200 bg-emerald-50/10' : 'border-slate-200'}`}
                        >
                          <div className="space-y-2">
                            <h4 className="font-black text-slate-800">{tag.campus}</h4>
                            <div className="flex justify-between text-xs font-bold">
                              <span>Required: {tag.minGpa.toFixed(2)}</span>
                              <span className={isEligible ? 'text-emerald-600' : 'text-indigo-600'}>Current: {calculatedMetrics.transferGPA.toFixed(2)}</span>
                            </div>
                          </div>
                          <div className={`p-2.5 rounded-xl text-[11px] font-bold flex items-center gap-1.5 ${isEligible ? 'bg-emerald-100/60 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>
                            ...
                            <span>{isEligible ? 'Eligible Baseline' : 'Requirements Unmet'}</span>
                          </div>
                        </div>
                      );
                    })}
                ...
                <div className="space-y-6">
                  {!isPremium ? (
                    <div className="max-w-md mx-auto text-center p-8 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-4">
                      ...
                      <button
                        onClick={() => setCurrentView('checkout')}
                        className="bg-indigo-600 text-white font-bold px-5 py-2.5 rounded-xl text-xs"
                      >
                        Unlock Pro Access
                      </button>
                    </div>
                          ...
                          majorRequirementsDB[customMajorName].map((req, idx) => {
                            ...
                            return (
                              <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between text-xs font-bold gap-2">
                                ...
                                <span className={isMet ? 'text-emerald-600 flex items-center gap-1' : isIP ? 'text-amber-600 flex items-center gap-1' : 'text-slate-400'}>
                                  {isMet ? (
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => { setCustomMajorName(e.target.value); saveUserData({ customMajorName: e.target.value }); }}
                      />
                    </div>
                    <div>
                ...
                <div className="w-80 h-96 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                  <div className="bg-slate-900 text-white p-3.5 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-extrabold tracking-wide">PathMaker AI Assistant</span>
                    </div>
                    <button onClick={() => setIsAiWidgetExpanded(false)} className="text-slate-400 hover:text-white">
                  ...
                  <div className="flex-1 p-3 overflow-y-auto space-y-2.5 bg-slate-50 text-[11px] font-medium leading-relaxed">
                    ...
                    {isAiResponding && (
                      <span className="text-[10px] text-slate-400 block animate-pulse">Assistant compiling response parameters...</span>
                    )}
                  ...
                  <div className="p-2.5 border-t border-slate-100 bg-white flex gap-1.5 shrink-0">
                    <input
                      type="text"
                      placeholder="Ask advisor about next courses..."
                      className="flex-1 px-3 py-1.5 border border-slate-200 rounded-xl bg-slate-50 text-xs font-bold focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      value={currentInput}
